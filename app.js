...
const googleConfig = {
  clientID:
    "850762646572-s418sekf7imkalnn764bg56odhluavuf.apps.googleusercontent.com",
...
mongoose
  .connect(
    "mongodb+srv://ritik:BIGGBOss12@cluster0.m96r8.gcp.mongodb.net/testing?retryWrites=true&w=majority",
    {
  ...
  .then(() => {
    console.log(
      "connected to db and public directory in" + __dirname + "/public"
    );
  })
  .catch((err) => {
  ...
  if (req.params.id == "0") {
    ...
    user.find(obj, function (err, user) {
      ...
      if (err || user[0] == undefined) {
        ...
        user[0].Role.is10DemProuser == true &&
        user[0].Role.isEducator == true &&
        user[0].Role.isNPOrg == true &&
        user[0].Role.isOrg
      ) {
        sess.req = session;
        ...
        if (
          ...
          educatoror10dempro.find(
            { _id: user[0].Role_object_id },
            function (err, role) {
              if (err) {
                console.log(
                  "Cant find educator or 10Dem pro user because: " + err
                );
              } else {
                sess.req = session;
...
/*####################################### 
  ...
  #######################################*/
app.post(
  "/superadmin/create-project",
  upload.single("projectCover"),
  (req, res) => {
    if (sess.user_data == undefined) {
      res.redirect("/");
    } else {
    ...
    .then(async () => {
      ...
      if (sess.user_data == undefined) {
        ...
        if (sess.user_data.user.Role_object_id == "") {
          ...
          res.render("index", {
            name: sess.user_data.user.username,
            firstletter: first_letter[0],
            hide_manage_students: true,
            org_name: "",
            role: "none",
            projects: projects,
            notifications: sess.user_data.user.notifications,
          });
    ...
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        user.find({}, (err, resp) => {
          if (err) {
            console.log(
              "Some kind of error loading users for superadmindashboard:- " +
                err
            );
            res.render("somethingWrong", { error: err });
          } else {
            resp.forEach((user_from_array) => {
              ...
              if (user_from_array.Role.isNPOrg == true) {
                ...
                console.log(
                  "Users notification:- " + user_from_array.notifications
                );
              ...
              console.log(
                "Count of users for superadmin dashboard:- " +
                  resp.length +
                  " and count is :- " +
                  count +
                  " their role is:- " +
                  user_from_array.Role
              );
              if (count == resp.length) {
                ...
                project.find({}, (err, resp1) => {
                  if (err) {
                    console.log(
                      "Cannot find projects for superadmin because:- " + err
                    );
                    res.render("somethingWrong", { error: err });
                  } else {
                    resp1.forEach(async (project_from_array) => {
                      count1++;
                      if (
                        project_from_array.created_by ==
                        "5f964cdc52f7629f909c85dc"
                      ) {
                        total_count_of_10dem_projects++;
                        if (
                          project_from_array.status == true &&
                          project_from_array.collaboration.length == 0
                        ) {
                      ...
                      if (count1 == resp1.length) {
                        ...
                        res.render("superadminDashboard", {
                          tnm: resp.length,
                          tnpm: total_count_of_pro_members,
                          tnom: total_count_of_org_members,
                          tnnm: total_count_of_nporg_members,
                          tnp: resp1.length,
                          tn10p: total_count_of_10dem_projects,
                          tndp: total_count_of_10dem_drafts,
                          tnop: total_count_of_10dem_ongoing,
                          tnep: total_count_of_10dem_ext_collb,
                          noom: notifications_of_org_members,
                          nonpm: notifications_of_nporg_members,
                          nopm: notifications_of_pro_members,
                          nofm: notifications_of_free_users,
                          projects: Sprojects,
                          drafts: drafts,
                        });
                      }
    ...
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        PorNPORG.find({}, (err, resp) => {
          if (err) {
            console.log(
              "Cannot find users in /superadmin/dashboard/orgmembership-Users because:- " +
                err
            );
            res.render("somethingWrong", { error: err });
          } else {
            resp.forEach((users) => {
              ...
              console.log(
                "Total users :- " + resp.length + " count:- " + count
              );
              if (count == resp.length) {
                console.log(
                  "Data sent in /superadmin/dashboard/orgmembership-Users is:- " +
                    all_org_users
                );
                res.render("superadminOrgDetails", {
                  orgdetails: all_org_users,
                });
              }
            });
          }
      ...
      if (sess.user_data == undefined) {
        ...
        user.find({}, (err, resp) => {
          resp.forEach((users) => {
            count++;
            if (
              users.Role.is10DemProuser == false &&
              users.Role.isEducator == false &&
              users.Role.isNPOrg == false &&
              users.Role.isOrg == false
            ) {
              all_free_users.push(users);
            }
            console.log(
              "Count going on:- " + count + " total users:- " + resp.length
            );
            if (count == resp.length) {
              res.render("superadminFreeDetails", {
                freedetails: all_free_users,
              });
            }
          });
        });
              ...
              classes.findById(document, function (err, single_class) {
                ...
                console.log(
                  "Count:- " +
                    count +
                    " Classes length:- " +
                    sess.user_data.role_Data.classes.length
                );
                if (count == sess.user_data.role_Data.classes.length) {
                  res.render("educatorDashboard", {
                    classesare: classes_are,
                    firstletter: first_letter[0],
                    name: sess.user_data.user.username,
                    role: "educator",
                    orgname: sess.user_data.role_Data.org_name,
    ...
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        classes.create(obj, (err, resp) => {
          if (err) {
            console.log(
              "error creating classes for org admin because :- " + err
            );
            res.render("somethingWrong", { error: err });
          } else {
            ...
            console.log(obj);
            PorNPORG.findOneAndUpdate(
              { _id: sess.user_data.role_Data._id },
              { $set: obj },
              { new: true },
              (err, resp) => {
                if (err) {
  ...
  internetConnected()
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        educatoror10dempro.findOneAndUpdate(
          { _id: sess.user_data.user.Role_object_id },
          { $set: obj },
          { new: true },
          function (err, resp) {
            if (err) {
    ...
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        classes.find({ _id: req.params.class }, (err, resp) => {
          if (err) {
            console.log(
              "Cannot find that class in /managestudent/ because:- " + err
            );
            res.render("somethingWrong", { error: err });
          } else {
            console.log(resp[0]);
            if (resp[0].students.length == 0) {
              ...
              resp[0].students.forEach((studentid) => {
                ...
                Student.find({ _id: studentid }, (err, resp1) => {
                  if (err) {
                    console.log(
                      "cannot find students of this class in managestudents/:class:- " +
                        err
                    );
                    res.render("somethingWrong", { error: err });
                  } else {
              ...
              classes.find({ _id: classesare }, (err, resp1) => {
                ...
                if (count == sess.user_data.role_Data.classes.length) {
                  ...
                  res.render("editStudent", {
                    ...
                    org_name: sess.user_data.role_Data.org_name,
                    hide_manage_students: false,
                    students_Data: resp[0],
                    classes_available: classes_Data,
                  });
                }
              });
            });
...
app.post("/addingstudent/:grade/:section/:id", (req, res) => {
  if (sess.user_data == undefined) {
    ...
    Student.create(obj, function (err, resp) {
      if (err) {
        console.log(
          "Cannot upload due to this error in /addingstudent/:grade/:section :- " +
            err
        );
        res.render("somethingWrong", { error: err });
      } else {
            ...
            if (err) {
              ...
              response.forEach((details) => {
                ...
                Student.create(obj, function (err1, resp1) {
                  if (err1) {
                    console.log(
                      "Cannot create student while reading from csv:- " + err1
                    );
                    ...
                    if (count == response.length) {
                      ...
                      classes.findOneAndUpdate(
                        ...
                        (err, resp) => {
                          if (err) {
                            console.log(
                              "Cannot update class after creating student while reading csv:- " +
                                err
                            );
                            res.render("somethingWrong", { error: err });
                          } else {
                            fs.remove(req.file.path, (err) => {
                              if (err) {
                                console.log(
                                  "Couldnt delete the file because:- " + err
                                );
                                res.render("somethingWrong", { error: err });
                              } else {
                                console.log("Deleted!");
            ...
            if (err) {
              ...
              if (resp[0].classes.length == 0) {
                if (resp[0].Educators.length == 0) {
                  res.render("organizationClass", {
                    classesare: [],
                    educators: resp[0].Educators,
                    firstletter: first_letter[0],
                    name: sess.user_data.user.username,
                    role: sess.user_data.user.Role,
                    orgname: resp[0].org_name,
                    notifications: sess.user_data.user.notifications,
                  });
                } else {
                  res.render("organizationClass", {
                    ...
                    if (err) {
                      ...
                      console.log(
                        "count:- " +
                          count +
                          " total classes:- " +
                          resp[0].classes.length
                      );
                      console.log(
                        "Number of Educators:- " + resp[0].Educators.length
                      );
                      if (count == resp[0].classes.length) {
                        res.render("organizationClass", {
                          ...
                          orgname: resp[0].org_name,
                          role: sess.user_data.user.Role,
                          educators: resp[0].Educators.length,
                          notifications: sess.user_data.user.notifications,
                        });
                      }
                    }
  ...
  internetConnected()
    .then(async () => {
      if (sess.user_data == undefined) {
        ...
        res.render("organizationEducator", {
          role: sess.user_data.user.Role,
          org_name: resp[0].org_name,
          name: sess.user_data.user.username,
          data: resp[0],
          educators: req_data,
        });
  ...
  internetConnected()
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        if (
          sess.user_data.role_Data.classes.length == 0 &&
          sess.user_data.role_Data.classes.length == 0
        ) {
          ...
          sess.user_data.role_Data.classes.forEach(function (document) {
            console.log(
              "Document incoming from foreach in line 490:- " + String(document)
            );
            classes.find({ _id: String(document) }, function (err, resp) {
              if (err) {
                console.log(
                  "inside /orgdashboard else loop cant find educators:- " + err
                );
                res.render("somethingWrong", { error: err });
              } else {
                ...
                count++;
                console.log(
                  count + " " + sess.user_data.role_Data.classes.length
                );
                if (count == sess.user_data.role_Data.classes.length) {
                  ...
                  res.render("admindashboard2", {
                    ...
                    totalnoofclasses: sess.user_data.role_Data.classes.length,
                    totalnoofeducators:
                      sess.user_data.role_Data.Educators.length,
                    totalnoofstudents: 0,
  ...
  internetConnected()
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        console.log(
          "getting in here:-" + sess.user_data.role_Data.classes.length
        );
        if (sess.user_data.role_Data.classes.length == 0) {
          ...
          sess.user_data.role_Data.classes.forEach(function (document) {
            classes.find({ _id: document }, function (err, resp) {
              if (err) {
                console.log(
                  "error in else loop /classDetails cannot find classes:- " +
                    err
                );
                res.render("somethingWrong", { error: err });
              } else {
...
/*################################
  ...
  ################################ */
app.post(
  "/orgdashboard/educators/uploadcsv",
          ...
          if (error) {
            ...
            response.forEach((row) => {
              ...
              educatoror10dempro.create(obj, (err, resp) => {
                if (err) {
                  console.log(
                    "Couldnt create educator with this data because:- " + err
                  );
                  res.render("somethingWrong", { error: err });
                } else {
                  ...
                  user.create(innerobj, (err, resp) => {
                    if (err) {
                      console.log(
                        "Error while creating educator user because:- " + err
                      );
                      res.render("somethingWrong", { error: err });
                    } else {
                      count++;
  ...
  internetConnected()
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        console.log(sess.user_data);
        let first_letter = sess.user_data.user.username.split("");
        res.render("adminProfile1", {
          id: sess.user_data.user._id,
          name: sess.user_data.user.username,
          orgname: sess.user_data.role_Data.org_name,
          phonenumber: sess.user_data.role_Data.phone_Number,
          email: sess.user_data.user.email,
        });
      }
    })
    .catch(() => {
  ...
  internetConnected()
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        res.render("adminProfile2", {
          id: sess.user_data.role_Data._id,
          name: sess.user_data.user.username,
          email: sess.user_data.user.email,
          img: sess.user_data.role_Data.img,
          orgname: sess.user_data.role_Data.org_name,
          phonenumber: sess.user_data.role_Data.phone_Number,
        });
      }
...
app.post("/change-password/:id", async (req, res) => {
  if (sess.user_data == undefined) {
    ...
    if (
      req.body.Current === sess.user_data.user.password &&
      req.body.newPassword == req.body.Confirm
    ) {
      user
        .findOneAndUpdate(
          { _id: sess.user_data.user._id },
    ...
    educatoror10dempro.create(obj, function (err, created_educator) {
      if (err) {
        ...
        user.create(obj1, function (err, created_user_of_educator) {
          if (err) {
            console.log(
              "Cannot create user for this specific educator in line 679:- " +
                created_user_of_educator
            );
            res.redirect("/orgdashboard");
          } else {
            var mailOptions = {
              ...
              subject:
                "You have been registered as an Educator at 10Dem by:- " +
                req.params.orgname,
              text:
                "Here is your email:- " +
                req.body.Email_id +
    ...
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        if (sess.user_data.role_Data.Educators.length == 0) {
          ...
          console.log(
            "Getting in /adminexternalcollab else else condition.Number of Educators are :- " +
              sess.user_data.role_Data.Educators.length
          );
          let ext_collb_proj = [];
          let count = 0;
              ...
              if (educator[0].Projects.length == 0) {
                ...
                console.log(
                  "Count:- " +
                    ...
                    " number of educators:- " +
                    sess.user_data.role_Data.Educators.length +
                    " number of projects of this educator:- " +
                    educator[0].Projects.length
                );
                educator[0].Projects.forEach(function (document1) {
                  project.findById(document1, function (err, resp) {
                    ...
                    if (err) {
                      console.log(
                        "Error while getting projects in /adminexternalcollabs in else else else:- " +
                          err
                      );
                      res.render("somethingWrong", { error: err });
                    } else {
                      ...
                      console.log(
                        "Count at if condition:- " +
                          count +
                          " Number of Educators:- " +
                          sess.user_data.role_Data.Educators.length +
                          " count1 at if condition:- " +
                          count1 +
                          " projects length of this educator:- " +
                          educator[0].Projects.length
                      );
                      if (
                        count == sess.user_data.role_Data.Educators.length &&
                        count1 == educator[0].Projects.length
                      ) {
                        console.log(
                          "##################################\n#################INCOMING PROJECTS#######################\n######################################\n" +
                            ext_collb_proj
      ...
      if (sess.user_data == undefined) {
        ...
        if (sess.user_data.user.Projects.length == 0) {
          if (sess.user_data.user.Role.is10DemProuser) {
            ...
            sess.user_data.user.Role.isNPOrg ||
            sess.user_data.user.Role.isOrg
          ) {
            res.render("myprojects", {
              name: sess.user_data.user.username,
              firstletter: first_letter[0],
            ...
            project.find({ _id: document_id }, function (err, resp) {
              ...
              console.log(
                "count:- " +
                  count +
                  " length:- " +
                  sess.user_data.user.Projects.length
              );
              if (count == sess.user_data.user.Projects.length) {
                console.log(
                  "All projects being sent to myprojects:- " + wholedata
                );
                if (sess.user_data.user.Role.is10DemProuser) {
                  res.render("myprojects", {
                    name: sess.user_data.user.username,
        ...
        sess.user_data.user.Projects.forEach(function (document_id) {
          project.find({ _id: document_id }, function (err, resp) {
            ...
            if (count == sess.user_data.user.Projects.length) {
              ...
              if (sess.user_data.user.Role.is10DemProuser) {
                res.render("externalcollaboration", {
                  name: sess.user_data.user.username,
                  firstletter: first_letter[0],
                  hide_manage_students: false,
                  org_name: "",
                  role: "educator",
                  ext_collb_proj: wholedata,
                });
              } else if (sess.user_data.user.Role.isEducator) {
                res.render("externalcollaboration", {
                  name: sess.user_data.user.username,
      ...
      if (sess.user_data == undefined) {
        ...
        if (req.params.id == 1) {
          if (sess.user_data.user.Role_object_id == "") {
            ...
            let first_letter = sess.user_data.user.username.split("");
            if (
              sess.user_data.user.Role.is10DemProuser == true ||
              sess.user_data.user.Role.isEducator == true
            ) {
              res.render("createproject1n", {
                name: sess.user_data.user.username,
                firstletter: first_letter[0],
      ...
      if (req.params.id == 1) {
        ...
        project.findOneAndUpdate(
          ...
          function (err, resp) {
            if (err) {
              console.log(
                "Inside post req from createproject2n and cant update project :- " +
                  err
              );
              res.render("somethingWrong", { error: err });
            } else {
              console.log(
                "Project updated redirecting to createproject3n: " +
                  util.inspect(resp)
              );
        ...
        console.log(
          "Attached files after for loop:- " + util.inspect(attachedfiles)
        );
        let data_to_be_updated = {
        ...
        project.findOneAndUpdate(
          ...
          function (err, resp) {
            if (err) {
              console.log(
                "Inside post req from createproject3n and cant update project:- " +
                  err
              );
              res.render("somethingWrong", { error: err });
            } else {
              console.log(
                "Project created with all this data and then redirecting to preview page:- " +
                  resp
              );
  ...
  internetConnected().then(async () => {
    if (sess.user_data == undefined) {
      ...
      if (
        ...
        users.Role.is10DemProuser == true ||
        users.Role.isEducator == true ||
        users.Role.isNPOrg == true ||
        users.Role.isOrg == true
      ) {
        var classData = [];
        ...
        res.render("ProjectOverviewpage", {
          project: project,
          classes: classData,
        });
      }
  ...
  function (req, res) {
    if (sess.user_data == undefined) {
      ...
      if (req.params.number == 1) {
        console.log(
          "Inside updating created project number 1:- " +
            util.inspect(req.body) +
            "\n############################\n" +
            util.inspect(req.files)
        );
        ...
        project.findOneAndUpdate(
          ...
          function (err, resp) {
            if (err) {
              console.log(
                "Inside Updating Created Projects getting this error:- " + err
              );
              res.render("somethingWrong", { error: err });
            } else {
              console.log(
                "Redirecting from updating created projects to project preview."
              );
        ...
        console.log(
          "Inside updating created project number 2:- " + util.inspect(req.body)
        );
        project.find({ _id: req.params.id }, function (err, resp) {
          let attached_files_now = resp[0].attached_files;
          console.log(
            "Attached files in system before appending new data:- " +
              attached_files_now
          );
          let comments_now = resp[0].comments;
          console.log(
            "Comments in system before appending new data:- " + comments_now
          );
          ...
          console.log(
            "Attached files in system after appending new data:- " +
              attached_files_now
          );
          comments_now.push({
          ...
          if (err) {
            ...
            if (sess.user_data.user.Role_object_id == "") {
              ...
              res.render("editproject", {
                firstletter: first_letter[0],
                projectidforurl: req.params.id,
                hide_manage_students: true,
                role: "none",
                org_name: "",
                title: resp[0].project_title,
                subject: resp[0].subject,
                grade: resp[0].grade,
                summary: resp[0].project_summary,
                learningoutcome: resp[0].learning_outcome,
                keycontri: resp[0].key_contribution,
                detailsactivity: resp[0].details_activity,
                files: attachedfiles,
                startdate: resp[0].start_date,
                enddate: resp[0].end_date,
              });
            } else if (
              sess.user_data.user.Role.is10DemProuser == true ||
              sess.user_data.user.Role.isEducator == true
            ...
            project.find({ _id: bookmark }, function (err, resp) {
              if (err) {
                ...
                if (sess.user_data.user.Bookmarks.length == count) {
                  console.log(
                    "All projects being sent to myprojects:- " +
                      util.inspect(projects_to_be_shown)
                  );
                  if (sess.user_data.user.Role.is10DemProuser) {
                    res.render("bookmarks", {
                      name: sess.user_data.user.username,
  ...
  if (sess.user_data == undefined) {
    ...
    user.find({}, function (err, resp) {
      if (err) {
          ...
          if (!resp[i]._id.equals(sess.user_data.user._id)) {
            ...
            user.findOneAndUpdate(
              ...
              function (err, respo) {
                if (err) {
                  console.log(
                    "Error in finding and updating notifications:- " + err
                  );
                  res.render("somethingWrong", { error: err });
                } else {
                  console.log("done");
        ...
        console.log(
          "Current collabs with appending " + util.inspect(collaborators)
        );
  ...
  internetConnected()
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        if (sess.user_data.user.Role_object_id == "") {
          let first_letter = sess.user_data.user.username.split("");
          res.render("membershiporganization", {
            firstletter: first_letter[0],
          });
        } else {
          res.send("<h1>Not ready yet!</h1>");
    ...
    .then(() => {
      if (sess.user_data == undefined) {
        ...
        if (req.params.id == 0) {
          ...
          res.render("payment", {
            id: req.params.id,
            membershipname: "Non- profit Organization",
            price: 0,
            name: sess.user_data.user.username,
            email: sess.user_data.user.email,
          });
  ...
  if (sess.user_data == undefined) {
    ...
    if (req.params.id == 0) {
      ...
      educatoror10dempro.create(obj, function (err, educatorordempro) {
        if (err) {
          console.log(
            "In /role/:id creating educatoror10Dempro and error is: " + err
          );
          res.render("somethingWrong", { error: err });
        } else {
          console.log(
            "Educatoror10dempro user created! with this data:- " +
              educatorordempro
          );
          let update = {
            username: req.body.Name,
          ...
          user.findOneAndUpdate(
            ...
            function (err, updatedresult) {
              if (err) {
                console.log(
                  "In /role/:id updating user and error is: " +
                    util.inspect(err)
                );
                res.render("somethingWrong", { error: err });
              } else {
                user.find(
                  ...
                  function (err, actual_data_of_user) {
                    console.log(
                      "User updated after creating educatorordempro user " +
                        actual_data_of_user
                    );
            ...
            function (err, updatedresult) {
              if (err) {
                ...
                user.find(
                  ...
                  function (err, actual_data_of_user) {
                    console.log(
                      "User updated after creating Org user " +
                        actual_data_of_user
                    );
                    sess.user_data.user = actual_data_of_user[0];
      ...
      PorNPORG.create(obj, function (err, NPOrg) {
        if (err) {
          console.log(
            "In /role/:id creating Non-Profit Org and error is: " + err
          );
          res.render("somethingWrong", { error: err });
        } else {
          ...
          user.findOneAndUpdate(
            { username: sess.user_data.user.username },
            { $set: update },
            { new: true },
            function (err, updatedresult) {
              if (err) {
                ...
                user.find(
                  ...
                  function (err, actual_data_of_user) {
                    console.log(
                      "User updated after creating NPOrg user " +
                        actual_data_of_user
                    );
          ...
          if (err) {
            ...
            if (sess.user_data.user.Role_object_id == "") {
              ...
              res.render("projectoverview", {
                sessuserid: sess.user_data.user._id,
                firstletter: first_letter[0],
                name: sess.user_data.user.username,
                projectidforurl: req.params.id,
                hide_manage_students: true,
              ...
              sess.user_data.user.Role.is10DemProuser ||
              sess.user_data.user.Role.isEducator
            ) {
              console.log("Attached files being sent:- " + attachedfiles);
              res.render("projectoverview", {
                sessuserid: sess.user_data.user._id,
